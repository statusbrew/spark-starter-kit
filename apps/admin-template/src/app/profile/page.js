...
export default function Profile() {
  const [mallFormData, setMallFormData] = useState({
    ...
    mallLocation: '',
    contactSupport: '',
    preHead: '', // Store the preHead here
  });
  ...
  // Extract the subdomain (preHead) on the client-side
  useEffect(() => {
    const host = window.location.href;
    const splitHost = host.split('//')[1].split('.')[0];
    

    let preHead = splitHost;
    console.log(preHead);

    // if (splitHost.length === 3) {  // Format: subdomain.domain.tld
    //   preHead = splitHost[0];
    // } else if (splitHost.length === 2 && host.includes('localhost')) {
    //   preHead = 'localhost';
    // }
  ...
  const submitParkingFee = async (e) => {
    ...
    paymentFormData.preHead = preHead;
  ...
  const submitParkingLayout = async (e) => {
    ...
    parkingFormData.preHead = preHead;
  ...
  const submitMallDetails = async (e) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/admin/register/mallDetails`, {
        ...
        body: JSON.stringify(mallFormData), // preHead is already part of mallFormData now
      });
